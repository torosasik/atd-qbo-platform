...
async function aiReview(data, realmId) {
  ...
  const aiResult = await askAI(prompt, { module: 'bill' });
  ...
  await logAction('bill', 'ai-review', flagged ? 'flagged' : 'clean', {
...
function buildPayload(data, matchedVendor) {
  ...
  // Build payload for QBO Bill entity
  ...
  // DocNumber (bill/reference number) — optional, QBO can auto-generate
  if (data.billNumber && typeof data.billNumber === 'string' && data.billNumber.trim() !== '') {
    payload.DocNumber = data.billNumber.trim();
  }
...
async function pushToQBO(payload, realmId) {
  ...
  try {
    ...
    const url = `${baseUrl}/v3/company/${realmId}/bill?minorversion=65`;
    ...
    if (!response.ok) {
      ...
      await logAction('bill', 'push-to-qbo', 'error', {
        intuitTid,
    ...
    const entityId = responseBody?.Bill?.Id;

    const qboTotal = responseBody?.Bill?.TotalAmt;
    await logAction('bill', 'push-to-qbo', 'success', {
      intuitTid,
      entityId,
      vendorName: payload.VendorRef?.name || null,
      billNumber: payload.DocNumber || null,
      total: qboTotal != null ? qboTotal : null,
    });

    return { success: true, data: responseBody.Bill, intuitTid };
  } catch (err) {
    await logAction('bill', 'push-to-qbo', 'error', {
      intuitTid,
...
async function handleCreate(req, res) {
  try {
    ...
    const docRef = await db.collection('bill_drafts').add({
      ...draftPayload,
      aiReview: aiReviewResult,
      status: 'pending',
    ...
    await logAction('bill', 'handle-create', 'error', { errorMessage: err.message });
...
async function handleApproveDraft(req, res) {
  try {
    ...
    await logAction('bill', 'handle-approve-draft', 'error', { errorMessage: err.message });
    return res.status(500).json({ success: false, error: err.message });
  }
}
