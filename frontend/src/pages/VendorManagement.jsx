...
export default function VendorManagement() {
  ...
  async function loadMappings() {
    ...
    try {
      ...
      const normalized = loadedVendors.map((v) => ({ visible: true, ...v }));
      setVendors(normalized);
      setSavedVendors(JSON.parse(JSON.stringify(normalized)));
      setLastSynced(mappings.last_synced || null);
      setQboConnected(!!authResult.connected);

      if (normalized.length === 0 && authResult.connected) {
        handleSync();
      }
    } catch (err) {
  ...
  async function handleSync() {
    ...
    try {
      ...
      const normalized = vendorList.map((v) => ({ visible: true, ...v }));
      setVendors(normalized);
      setSavedVendors(JSON.parse(JSON.stringify(normalized)));
      ...
      if (normalized.length === 0) {
        ...
        showToast(`Synced ${normalized.length} vendors from QuickBooks.`);
      }
    } catch (err) {
  ...
  const filtered = useMemo(() => {
    let list = vendors.map((v, i) => ({ ...v, _idx: i }));
    if (!showHidden) {
      list = list.filter((v) => v.visible !== false);
    }
  ...
  const hiddenCount = vendors.filter((v) => v.visible === false).length;
            ...
            <div className="flex items-center gap-2">
              ...
              {hiddenCount > 0 && (
                <>
                  <span className="text-gray-300">|</span>
                  <Toggle
                    id="show-hidden-inline"
                    checked={showHidden}
                    onChange={setShowHidden}
                    label={showHidden ? 'Hiding hidden' : `Show Hidden (${hiddenCount})`}
                  />
                </>
              )}
            </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-xs font-semibold text-gray-500 uppercase tracking-wider border-b border-gray-100">
                  <th className="px-6 py-3 w-14">Active</th>
                  <th className="px-6 py-3 w-14">Visible</th>
                    ...
                    <tr key={v._idx} className={`hover:bg-gray-50 ${isHidden ? 'opacity-40' : ''}`}>
                      ...
                      <td className="px-6 py-3">
                        <Toggle
                          ...
                          onChange={() => toggleVisible(v._idx)}
                          ariaLabel={`Toggle visibility for ${v.qbo_name}`}
                        />
                      </td>
